...
let validateCred = arr => {
  ...
  // const sum = luhnArr.reduce((a, b) => a + b, 0);
...
let idInvalidCardCompanies = invalidCards => {
  ...
  for (let i = 0; i < invalidCards.length; i++) {
    switch(String(invalidCards[i][0])) {
  case '3':
  ...
  let uniqueCardCompanies = [...new Set(cardCompanies)]; // deleting duplicates by converting to a set
  let array = [...uniqueCardCompanies]; // changing set back to array
  return array;
}
